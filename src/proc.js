...
function wb(addr, val) //Write byte
{
	...
	if(addr == 0xFF80 || addr == 0xFF00) debug_out("Wrote to addr " + addr.toString(16) + " value " + val.toString(16) + " at PC " + (PC-1).toString(16));
	if(addr == 0xFF44)
	{
		mem[0xFF44] = 0;
...
{
	if(addr >= 0xE000 && addr <= 0xFE00) addr -= 0x2000; //Mirrored memory
	if(addr == 0xFF80 || addr == 0xFF00) debug_out("Read from addr " + addr.toString(16) + " at PC " + (PC-1).toString(16));
